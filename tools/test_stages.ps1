...
# (launcher.ps1, the same code the launcher window and the desktop shortcuts use), waits, takes screenshots (normal frame + optionally the motion-vector visualiser), collects a
...
foreach ($stage in $list) {
    ...
    $t0 = Get-Date
    & powershell -ExecutionPolicy Bypass -File (Join-Path $tools "launcher.ps1") $stage --start-timeout 60 --game-dir $GameDir 2>&1 | Select-Object -Last 1 | ForEach-Object { Log "  launcher: $_" }
