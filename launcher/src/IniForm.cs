...
namespace Mgs4Launcher
{
    ...
    static class IniForm
    {
        ...
        public static readonly List<IniKey> Spec = new List<IniKey>
        {
            ...
            new IniKey("Image", "DRSMin", "choice", "Dynamic resolution floor",
                "the lowest the game may take its own 3D scene, as a fraction of the resolution. The port scales the scene by its measured GPU time, and DLAA, NR and frame generation all count, so left alone it sits at 1920x1080 of 4K under load. Full size holds it there: the cost is frame rate rather than resolution when the GPU cannot keep up (Mode = Quality is the cheaper way). Live",
                new[] { "1.0", "0.75", "0.5", "0" },
                new[] { "Full size - never below the resolution (costs frame rate)", "75 %", "50 %", "The game's own (off)" }) { Fallback = "0" },
