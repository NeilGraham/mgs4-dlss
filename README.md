...
- **HUD on generated frames.** In pre-post insertion DLSS-G gets the anti-aliased image before the HUD as HUD-less colour. In composite insertion (DLSS 5 NR loaded) the HUD is inside the image, so the add-on replays the game's HUD draws (depth-off draws into the final texture that sample no scene-sized input) into its own RGBA layer and tags it as `UIColorAndAlpha` next to the DLAA output as HUD-less colour; DLSS-G then re-composites the HUD unwarped instead of interpolating it with the scene. Debug mode "Visualise UI layer" shows that layer; the overlay counts the replayed HUD draws per frame.
