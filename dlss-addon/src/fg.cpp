...
void frame_inputs(uint32_t frameIndex, const FrameInputs& in, const CameraInput& cam)
{
    ...
    sl::Resource depthRes(sl::ResourceType::eTex2d, in.depth, in.depthState); depthRes.width = in.texW ? in.texW : in.renderW; depthRes.height = in.texH ? in.texH : in.renderH; depthRes.nativeFormat = in.depthFormat;
    sl::Resource mvRes(sl::ResourceType::eTex2d, in.mv, in.mvState); mvRes.width = in.texW ? in.texW : in.renderW; mvRes.height = in.texH ? in.texH : in.renderH; mvRes.nativeFormat = DXGI_FORMAT_R16G16_FLOAT;
