...
// exactly what the game's pass would have sampled); the color is added at the DLSS insertion (dof_pack_cs), then the
...
cbuffer CB : register(b0)
{
    ...
    float  debugView;    // 0 = normal, 1 = blurred layer only, 2 = coverage (gray), 3 = overlay mask
