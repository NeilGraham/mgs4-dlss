...
void main(uint3 id : SV_DispatchThreadID)
{
    ...
    if (debugView > 0.5) {
        ...
        else { float4 mk = hasMask > 0.5 ? maskTex.Load(int3(int2(float2(id.xy) * maskScale), 0)) : 0.0; dbg.rgb = saturate(max(max(mk.r, mk.g), max(mk.b, mk.a)) * 2.0).xxx; }
    ...
    if (hasMask > 0.5) { float4 mk = maskTex.Load(int3(int2(float2(id.xy) * maskScale), 0)); keep = saturate(max(max(mk.r, mk.g), max(mk.b, mk.a)) * 2.0); }
